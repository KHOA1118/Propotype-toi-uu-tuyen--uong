/* Sample boundaries are deterministic; no global solver-time animation freeze. */
(function(root){
  class Presentation {
    ...
    summary(d){const a=d.before?.travel,b=d.after?.travel;if(a==null||b==null)return 'Không thể so sánh thời gian khi tuyến bị chặn.';const gain=a-b;return `${d.rerouted} xe đổi tuyến · ${gain>0?'Giảm':gain<0?'Tăng':'Không đổi'} ${Math.abs(gain).toFixed(1)} giây thời gian chạy dự kiến. Khách hàng giữ nguyên theo xe.`;}
  }
