...
test('three real road-cost LNS demos: hidden physics, two telemetry intervals, async solve, preserved progress, depot return',async()=>{
 ...
 for(const frame of [17,50,100]){
  ...
  assert.equal(initial.routes.length,6);assert.equal(initial.cost_unit,'seconds');assert.ok(initial.overlap_analysis.accepted);
  const geometry=initial.road_geometry, scenario=fixture.scenario;
  assert.ok(initial.routes.every(r=>r.length>=5&&r.length<=7));
  ...
  while(sim.elapsed<fixture.event.inject_at_ms)sim.advance(run.budget(sim.elapsed,frame));
  ...
  for(let n=0;n<500;n++){
   while(sim.elapsed<run.nextSample)sim.advance(run.budget(sim.elapsed,frame));
   ...
   assert.equal(measured.known_state.revision,0);
  }
  ...
  const result=await completed(queued,()=>sim.advance(100));
